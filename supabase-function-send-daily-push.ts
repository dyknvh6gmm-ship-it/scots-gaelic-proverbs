...
// has recorded. Mirrors supabase-function-send-daily-proverb.ts and the
// "proverb-data" JSON block embedded in index.html — if you add proverbs
// there, regenerate this list to match.
...
Deno.serve(async () => {
  ...
  // Only join together whichever languages this proverb actually has —
  // most entries are bilingual, not all three.
  const parts = [proverb.scots, proverb.gaelic, proverb.english].filter(Boolean);
  ...
  const res = await fetch("https://onesignal.com/api/v1/notifications", {
    ...
    body: JSON.stringify({
      ...
      contents: { en: parts.join("  ·  ") },
      url: "https://www.gaelicwithsteve.com/"
    })
  });
